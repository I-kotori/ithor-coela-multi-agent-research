...
최종 결과뿐 아니라 개발 과정에서 생성한 실행 영상, trace, 메모리, 프롬프트와 실행 로그를 함께 보존합니다. 각 `kitchen_prep` 결과 디렉터리에는 `ithor_coela_lite_demo.mp4`, `coela_lite_trace.json`, `agent_memories.json`, `llm_prompts.json`이 들어 있습니다.

| 구간 | 보존된 실험 |
|---|---|
| 초기 iTHOR 검증 | `ithor_output`, `ithor_output_v3`, `ithor_output_v3_safe`, `ithor_prompt_output` |
| 초기 CoELA-style | `ithor_coela_style_output`, `ithor_coela_lite_output` |
| 운반 과제 | `ithor_coela_lite_transport_output`, `ithor_coela_lite_transport_output_v2` |
| Kitchen-prep 초기 | `v3`, `v3_1080p`, `v4_steps`, `v5_steps`, `v7_prompt_cards` |
| v8 개선 실험 | `v8_final`, `v8_clean`, `v8_recovery`, `v8_release` |
| 장기 작업 개선 | `v9_open_surface` ~ `v15_separate_destinations` |
| 통신·시각화 개선 | `v16_comm_visible`, `v17_comm_motion` |
| 최종본 | `v18_1080p_slow_final` |

`checkpoints/`에는 초기 transport v2 및 kitchen-prep v3 구현 코드와 결과도 보존합니다. 버전 번호가 없는 구간이나 v6처럼 디렉터리가 남아 있지 않은 단계는 현재 로컬에 존재하는 자료만 기준으로 정리했습니다.
...
└── ithor_demo/
    ├── *.py                                  # 구현 및 실험 스크립트
    ├── *_run.log                             # 버전별 실행 결과
    ├── checkpoints/                          # 초기 코드·결과 체크포인트
    ├── ithor_output*/                        # 기본 다중 에이전트 실험
    ├── ithor_prompt_output/                  # 프롬프트 플래너 실험
    ├── ithor_coela_style_output/             # 초기 CoELA-style 실험
    ├── ithor_coela_lite*_output*/            # 초기·운반 과제 결과
    └── ithor_coela_lite_kitchen_prep_v*/     # v3~v18 영상·JSON 결과
```

약 106 MiB의 버전별 연구 산출물은 모두 Git 이력에 포함합니다. 약 5 GiB의 영상 생성용 raw JPG 프레임, Python 캐시, 163 MiB의 Miniconda 설치 파일만 제외했습니다. raw 프레임은 코드로 다시 생성할 수 있고 각 실험의 합성 MP4가 보존되어 있습니다.
